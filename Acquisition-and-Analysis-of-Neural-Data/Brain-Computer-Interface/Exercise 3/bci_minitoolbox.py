...
def scalpmap(mnt, v, clim='minmax', cb_label='', cmap='rainbow', ax=None, contours=False): 
    '''
    Usage:
    ...
    if ax == None:
        plt.imshow(zi, vmin=vmin, vmax=vmax, origin='lower', extent=[-1, 1, -1, 1], cmap=cmap)
        plt.colorbar(shrink=.5, label=cb_label, cmap=cmap)
        plt.scatter(mnt[:,0], mnt[:,1], c='k', marker='+', vmin=vmin, vmax=vmax)
        plt.axis('off')
        plt.contour(xi, yi, zi, colors='black', linewidths=1, linestyles='dashed') if contours else None
    else:
        im = ax.imshow(zi, vmin=vmin, vmax=vmax, origin='lower', extent=[-1, 1, -1, 1], cmap=cmap)
        ax.scatter(mnt[:,0], mnt[:,1], c='k', marker='+', vmin=vmin, vmax=vmax)
        ax.axis('off')
        ax.contour(xi, yi, zi, colors='black', linewidths=1, linestyles='dashed') if contours else None
    
    return im if ax != None else None
    plt.axis('off')
